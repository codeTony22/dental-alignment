...
from case_prep.domain.clock_signature import (canon_point_to_world, scan_rim_centre,
                                              template_signature)
from case_prep.domain.part_features import MIN_LEVER_ARM_MM, template_rim_centre
...
def measured_rim_centre_world(pose_world: np.ndarray,
                              template: trimesh.Trimesh) -> np.ndarray:
    """THE CLOCK-REFERENCE PIVOT ``application.adjust.require_clock_lever`` guards
    against (plan §10-F), in WORLD coordinates — for a PRE-RUN preview that has no
    shipped record to build the cached ``SiteContext`` ``adjust.site_clicks``
    reads from.

    ONE PIVOT (§10-AJ, the −17.1° ghost probe): ``adjust.site_clicks`` now pivots
    every angular read on the TEMPLATE's own rim centre — the same centre the
    feature azimuths, the signature and the applied rotation are expressed about —
    because measuring click azimuths about the scan's MEASURED rim centre carried
    parallax into every pair delta. This function follows: it is the pivot the
    lever guards read, and Declare's guard must agree with Adjust's on where a
    mark sits relative to the bound (the cross-check in test_preview.py holds the
    two paths identical on real fleet data). The scan's measured rim centre stays
    a qc/clock MEASUREMENT elsewhere; it is no longer this reference."""
    pose = np.asarray(pose_world, float)
    sig = template_signature(template)
    return canon_point_to_world(template_rim_centre(template), sig.ztop, pose)
