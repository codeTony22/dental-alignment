 ...
 * Pure numeric helper — no THREE/DOM dependency — kept private to this module since nothing
 * outside sceneController needs a general percentile function today.
 */
...
export class SceneController {
  ...
  private frameOnSphereParams(center: THREE.Vector3, radiusIn: number, fit: ViewerFitMode): void {
    const radius = Math.max(radiusIn, 0.01);
    const fovRadians = (this.camera.fov * Math.PI) / 180;
    const paddingFactor = fit === "close" ? 1.4 : 2.4;
  ...
  isToolActive(): boolean {
    return (
      this.isBrushActive() ||
      this.isMarkModeActive() ||
      this.isRimPointsActive() ||
      this.isPointPickActive()
    );
  }
  ...
  focusOnSite(
    ...
    const radius = Math.max(radiusMm, 0.01);
    const fovRadians = (this.camera.fov * Math.PI) / 180;
    ...
    // and not the cap.
    const distance = (radius * SITE_FIT_PADDING) / Math.sin(fovRadians / 2);
  ...
  frameLoadedContent(): boolean {
    ...
    const fovRadians = (this.camera.fov * Math.PI) / 180;
    const distance = (Math.max(radius, 0.01) * 2.4) / Math.sin(fovRadians / 2);
    const current = this.camera.position.clone().sub(this.controls.target);
    const direction =
  ...
  setAnatomyView(view: AnatomyViewId): void {
    ...
    this.setCameraUp(up);
    this.camera.position.copy(center).addScaledVector(direction.normalize(), this.lastFrameDistance);
    this.controls.target.copy(center);
    this.controls.update();
  }
  ...
  setPartFeatureMarkers(specs: readonly PartFeatureMarkerSpec[]): number {
    ...
    for (const spec of specs) {
      ...
      const radius = spec.selected ? FEATURE_MARKER_SELECTED_RADIUS_MM : FEATURE_MARKER_RADIUS_MM;
      const sphere = this.buildFeatureSphere(spec.kind, radius);
