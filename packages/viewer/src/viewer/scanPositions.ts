 ...
 *
 * DIVERGENCE from the frozen apps/web original (copy-debt ledger row 3): the demo resolved
 * the download URL itself through its api client's scanUrlFor; this package owns no API
 * client and no route shape, so the CALLER supplies the URL (the product passes its BFF
 * scan endpoint) and the cache stays keyed by case id exactly as before.
 */
import { loadStlPositions } from "./verifyScene";

let scanCache: { caseId: string; positions: Float32Array } | null = null;

export async function scanPositionsFor(caseId: string, scanUrl: string): Promise<Float32Array> {
  if (scanCache?.caseId === caseId) return scanCache.positions;
  const positions = await loadStlPositions(scanUrl);
